...
def test_update_zero_fills_trending_history_and_includes_last_date(monkeypatch):
    ...
    monkeypatch.setattr(
        stats.search, "create_or_update_apps", stop_after_trending_batch
    )
...
def test_stats(client):
    ...
    expected["updates_per_day"][yesterday.isoformat()] = 1480
    expected["updates_per_day"][today.isoformat()] = 772
...
def test_verification_domain_names():
    ...
    assert _get_domain_name("com.github.Example") == (None, False)
    assert _get_domain_name("com.gitlab.Example") == (None, False)
