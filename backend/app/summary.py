...
def update(sqldb) -> None:
    ...
    try:
        # Build a complete map of EOL apps and branches from the summary
        ...
        # Get all apps that are currently EOL in the DB
        db_eol_apps = set(models.App.get_eol_apps(sqldb))

        # The set of all apps we need to process is the union of EOL apps from the summary and from the DB
        ...
        for app_id in apps_to_process:
            ...
            if app_id in summary_eol_map:
                # App is EOL in the summary, so mark it as such and set its EOL branches
                app.is_eol = True
                app.eol_branches = list(summary_eol_map[app_id])
                sqldb.session.add(app)
            else:
                # App is not EOL in the summary, but was in the DB, so un-EOL it
                app.is_eol = False
