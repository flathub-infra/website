...
def calculate_trending_score(
    ...
    with localcontext() as context:
        context.prec = 28
        history = [Decimal(value) for value in installs_over_days]
        baseline = history[:14]
        recent = history[14:]
        ...
        adjusted_momentum = 20 * directional_growth * confidence * consistency_weight
        if adjusted_momentum > 0:
            ...
            quality_signal = (
                Decimal("0.75") * guideline_quality + Decimal("0.25") * icon_quality
            )
