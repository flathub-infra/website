...
const LoginServiceClient = ({ services }: LoginServiceClientProps): JSX.Element => {
  ...
  const loginQuery = useMutation({
    mutationFn: () => login(dispatch, query),
    onSuccess: () => {
    ...
    if (loginQuery.isIdle) {
      ...
      if (newlocale && typeof window !== "undefined") {
        ...
        if (newlocale !== currentLocale) {
          router.push(window.location.pathname.replace(`/${currentLocale}/`, `/${newlocale}/`))
          return
        }
      }
  ...
  useEffect(() => {
    const service = searchParams.get('service') || window.location.pathname.split('/').pop()
