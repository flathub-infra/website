...
// Note: Using inline styles instead of SCSS module for app router compatibility
import {
  HiCloudArrowDown,
  HiCalendar,
  HiListBullet,
  ...
  return (
    <div
      className={clsx(
        ...
        "[&_figure]:m-0 [&_figure]:rounded-xl [&_figure]:w-fit",
      )}
    >
      <WorldMap
        color={
        ...
        tooltipBgColor={
          resolvedTheme === "dark"
            ? "oklch(17.11% 0.006 286.75)"
            : "oklch(98.42% 0.003 106.47)"
        }
        tooltipTextColor={
          resolvedTheme === "dark"
            ? "oklch(83.13% 0.006 106.11)"
            : "oklch(37.99% 0.016 282.68)"
        }
        onClickFunction={(context: CountryContext) => {
          if (refs[context.countryCode]) {
            refs[context.countryCode].current?.scrollIntoView({
              behavior: "smooth",
            })
          }
        }}
